...
def main():
    ...
    # top_shapes_height = 30
    top_shapes_height = 50
    ...
    while True:
        ...
        while num_shapes < cycle_size:
            if num_shapes % 30:
                crop_level = grid.max_y - top_shapes_height
                for x in range(0, 7):
                    for y in range(grid.min_y, crop_level):
                        if (x, y) in grid.items:
                            del grid.items[(x, y)]

            if grid.items and grid.max_y > 250:
                new_items = {}
            ...
            if after_cycle_repeats:
                ...
                if shapes_after_cycles < 0:
                    ...
                    print('-' * 80)
                    print('-' * 80)
                    lines_after_cycles = grid.max_y + 100 * max_y_hundreds - lines_at_target_start
